...
func (w *Client) Micropay(bm sycpay.BodyMap) (wxRsp *MicropayResponse, err error) {
...
func (w *Client) AuthCodeToOpenId(bm sycpay.BodyMap) (wxRsp *AuthCodeToOpenIdResponse, err error) {
...
func (w *Client) UnifiedOrder(bm sycpay.BodyMap) (wxRsp *UnifiedOrderResponse, err error) {
...
func (w *Client) QueryOrder(bm sycpay.BodyMap) (wxRsp *QueryOrderResponse, err error) {
	err = bm.CheckEmptyError("nonce_str")
	if err != nil {
		return nil, err
	}
	if bm.Get("out_trade_no") == sycpay.NULL && bm.Get("transaction_id") == sycpay.NULL {
...
func (w *Client) CloseOrder(bm sycpay.BodyMap) (wxRsp *CloseOrderResponse, err error) {
...
func (w *Client) Reverse(bm sycpay.BodyMap, certFilePath, keyFilePath, pkcs12FilePath string) (wxRsp *ReverseResponse, err error) {
	err = bm.CheckEmptyError("nonce_str", "out_trade_no")
	if err != nil {
		return nil, err
	}
...
func (w *Client) Refund(bm sycpay.BodyMap, certFilePath, keyFilePath, pkcs12FilePath string) (wxRsp *RefundResponse, err error) {
...
func (w *Client) QueryRefund(bm sycpay.BodyMap) (wxRsp *QueryRefundResponse, err error) {
	err = bm.CheckEmptyError("nonce_str")
	if err != nil {
		return nil, err
	}
	if bm.Get("refund_id") == sycpay.NULL && bm.Get("out_refund_no") == sycpay.NULL && bm.Get("transaction_id") == sycpay.NULL && bm.Get("out_trade_no") == sycpay.NULL {
...
func (w *Client) DownloadBill(bm sycpay.BodyMap) (wxRsp string, err error) {
	...
	if billType != "ALL" && billType != "SUCCESS" && billType != "REFUND" && billType != "RECHARGE_REFUND" {
		return sycpay.NULL, errors.New("bill_type error, please reference: https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_6")
	}
	var bs []byte
	if w.IsProd {
...
func (w *Client) DownloadFundFlow(bm sycpay.BodyMap, certFilePath, keyFilePath, pkcs12FilePath string) (wxRsp string, err error) {
	...
	if accountType != "Basic" && accountType != "Operation" && accountType != "Fees" {
		return sycpay.NULL, errors.New("account_type error, please reference: https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_18&index=7")
	}
...
func (w *Client) Report(bm sycpay.BodyMap) (wxRsp *ReportResponse, err error) {
...
func (w *Client) BatchQueryComment(bm sycpay.BodyMap, certFilePath, keyFilePath, pkcs12FilePath string) (wxRsp string, err error) {
...
func (w *Client) Transfer(bm sycpay.BodyMap, certFilePath, keyFilePath, pkcs12FilePath string) (wxRsp *TransfersResponse, err error) {
	...
	httpClient := sycpay.NewHttpClient().SetTLSConfig(tlsConfig).Type(sycpay.TypeXML)
	if w.BaseURL != sycpay.NULL {
		w.mu.RLock()
...
func (w *Client) EntrustPublic(bm sycpay.BodyMap) (wxRsp *EntrustPublicResponse, err error) {
...
func (w *Client) EntrustAppPre(bm sycpay.BodyMap) (wxRsp *EntrustAppPreResponse, err error) {
	err = bm.CheckEmptyError("plan_id", "contract_code", "request_serial", "contract_display_account", "notify_url", "version", "timestamp")
	if err != nil {
		return nil, err
	}
...
func (w *Client) EntrustH5(bm sycpay.BodyMap) (wxRsp *EntrustH5Response, err error) {
...
func (w *Client) EntrustPaying(bm sycpay.BodyMap) (wxRsp *EntrustPayingResponse, err error) {
...
// Post请求
func (w *Client) doWeChatPostSanBox(bm sycpay.BodyMap, path string) (bs []byte, err error) {
	...
	res, bs, errs := sycpay.NewHttpClient().Type(sycpay.TypeXML).Post(url).SendString(generateXml(bm)).EndBytes()
	if len(errs) > 0 {
		return nil, errs[0]
	}
	if res.StatusCode != 200 {
...
// Post请求、正式
func (w *Client) doWeChatPostProd(bm sycpay.BodyMap, path string, tlsConfig *tls.Config) (bs []byte, err error) {
	var url = baseUrlCh + path
	w.mu.RLock()
	defer w.mu.RUnlock()
	bm.Set("appid", w.AppId)
	...
	res, bs, errs := httpClient.Type(sycpay.TypeXML).Post(url).SendString(generateXml(bm)).EndBytes()
	if len(errs) > 0 {
		return nil, errs[0]
	}
	if res.StatusCode != 200 {
...
// Get请求、正式
func (w *Client) doWeChatGetProd(bm sycpay.BodyMap, path, signType string) (bs []byte, err error) {
	var url = baseUrlCh + path
	w.mu.RLock()
	defer w.mu.RUnlock()
	bm.Set("appid", w.AppId)
	...
	url = url + "?" + param
	res, bs, errs := sycpay.NewHttpClient().Get(url).EndBytes()
	if len(errs) > 0 {
		return nil, errs[0]
	}
	if res.StatusCode != 200 {
